...
def _error_response(request, status_code, title, message, icon_svg=None):
    if icon_svg is None and status_code == 404:
        icon_svg = '''
        <svg xmlns="http://www.w3.org/2000/svg" height="140" width="140" viewBox="0 0 24 24">
        ...
        <div class="page-content error-page error2 text-white">
            <div class="container text-center">
                <div class="error-template">
                    <h1 class="display-1 mb-2">{code_html}</h1>
                    <div class="m-5">
                        <span class="fs-20">{title}</span>
                        <p>{message}</p>
                    </div>
                    <div class="text-center">
                        <a class="btn btn-secondary mt-5 mb-5" href="/">
                            بازگشت به صفحه اصلی
                        </a>
                    </div>
                </div>
